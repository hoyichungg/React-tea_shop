...
function _getConfig() {
  _getConfig = _asyncToGenerator(function* (opts) {
    ...
    webpackConfig.module.rule('js-in-node_modules').test(/\.(js|mjs)$/).include.add(/node_modules/).end() // TODO: 处理 tnpm 下 @babel/runtime 路径变更问题
    .exclude.add(path => {
      return (0, _shouldExclude.default)({
        path
      });
    }).end().use('babel-loader').loader(require.resolve('babel-loader')).options((0, _bundlerUtils().getBabelDepsOpts)({
      cwd,
      env,
      config
    })); // prettier-ignore
