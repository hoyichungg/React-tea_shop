...
function _default({
  ...
  function patchRoute(route) {
    if (route.component && !isFunctionComponent(route.component)) {
      const webpackChunkName = route.component.replace(new RegExp(`^${lastSlash((0, _utils().winPath)(cwd || '/'))}`), '').replace(/^.(\/|\\)/, '').replace(/(\/|\\)/g, '__').replace(/\.jsx?$/, '').replace(/\.tsx?$/, '').replace(/^src__/, '') // 约定式路由的 [ 会导致 webpack 的 code splitting 失败
